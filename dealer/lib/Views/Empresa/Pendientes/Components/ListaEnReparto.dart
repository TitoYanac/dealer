...
import 'EnReparto/popup.dart';
import 'EnReparto/popup_body.dart';
import 'EnReparto/popup_content.dart';
...
class _EnviosPendientesState extends State<EnviosPendientes> {
  Future cargarEnviosEnReparto() async {
              ...
              itemBuilder: (context, index) {
                return Container(
                  ...
                  child: ListTile(
                    ...
                    title: Text(
                      "${data[index]['EMPRESA'].toUpperCase()}",
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                      ),
                    ),
                    subtitle: Text(
                        "${miDistrito[int.parse(data[index]['ORIGEN_ID_DISTRITO'])]} → ${miDistrito[int.parse(data[index]['DESTINO_ID_DISTRITO'])]}"),
                    trailing: Text(
                        "${DateTime.parse(data[index]['FECHA_CREACION']).add(Duration(hours: 3)).difference(DateTime.now())}"),
                    dense: true,
                    onTap: () {
                      showPopup(context, PopUpBody(element: data[index]),
                          data[index]);
                    },
                  ),
                );
